...
@login_required
def index():
    filter_date_str = request.args.get('date', date.today().isoformat())
    try:
...
def book():
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Invalid date. Please select a valid appointment date.', 'error')
...
def barber_slots():
    ...
    try:
        appt_date = datetime.strptime(request.args.get('date', ''), '%Y-%m-%d').date()
    except (ValueError, TypeError):
        return jsonify([])
